...
__global__ void multihead_attention_fp16_kernel(int   pos,
                                                half* sq,
                                                half* sxb,
                                                half* key_cache,
                                                half* value_cache,
...
void multihead_attention_fp16(half* q,
                              half* xout,
                              half* k_cache,
                              half* v_cache,
                              int   n_heads,
                              ...
                              int   loff)
{
    multihead_attention_fp16_kernel<<<n_heads, BLOCK_SIZE * BLOCK_SIZE>>>(
        pos, q, xout, k_cache, v_cache, kv_dim, kv_mul, head_size, loff, 1.0 / sqrt(head_size));
    CHECK_CUDA(cudaGetLastError());
}

void multihead_attention_fp16_gpu(float* q,
                                  float* xout,
                                  float* k_cache,
                                  float* v_cache,
                                  int    n_heads,
                                  ...
                                  int    loff)
{
    ...
    CHECK_CUDA(cudaMemcpy(k_cache_d,
                          float_to_half(k_cache, n_layers * seq_len * kv_dim),
                          sizeof(half) * n_layers * seq_len * kv_dim,
                          cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(v_cache_d,
                          float_to_half(v_cache, n_layers * seq_len * kv_dim),
                          sizeof(half) * n_layers * seq_len * kv_dim,
                          cudaMemcpyHostToDevice));
...
__global__ void multihead_attention_kernel(int    pos,
                                           float* sq,
                                           float* sxb,
                                           float* key_cache,
                                           float* value_cache,
...
void multihead_attention_fp32(float* q,
                              float* xout,
                              float* k_cache,
                              float* v_cache,
                              int    n_heads,
                              ...
                              int    loff)
{
    multihead_attention_kernel<<<n_heads, BLOCK_SIZE * BLOCK_SIZE>>>(
        pos, q, xout, k_cache, v_cache, kv_dim, kv_mul, head_size, loff, 1.0 / sqrt(head_size));
    CHECK_CUDA(cudaGetLastError());
}

void multihead_attention_fp32_gpu(float* q,
                                  float* xout,
                                  float* k_cache,
                                  float* v_cache,
                                  int    n_heads,
...
void multihead_attention_fp32_cpu(float* q,
                                  float* xout,
                                  float* k_cache,
                                  float* v_cache,
                                  int    n_heads,
...
void test_attn(bool verbose)
{
    ...
    multihead_attention_fp32_cpu(
        q, xout_fp32_cpu, k_cache, v_cache, n_heads, n_layers, seq_len, dim, pos, kv_dim, kv_mul, head_size, loff);

    float* xout_fp32_gpu = alloc_mat(dim, 1);
    multihead_attention_fp32_gpu(
        q, xout_fp32_gpu, k_cache, v_cache, n_heads, n_layers, seq_len, dim, pos, kv_dim, kv_mul, head_size, loff);

    float* xout_fp16_gpu = alloc_mat(dim, 1);
    multihead_attention_fp16_gpu(
        q, xout_fp16_gpu, k_cache, v_cache, n_heads, n_layers, seq_len, dim, pos, kv_dim, kv_mul, head_size, loff);
